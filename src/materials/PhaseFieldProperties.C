...
PhaseFieldProperties::computeQpProperties()
{
  ...
  _lambda[_qp] = (_a1 * w * (convert_meters) / _capillary_length[_qp]);

  _tau[_qp] = (_beta[_qp] * w * (convert_meters) * _lambda[_qp]) / _a1;
  ...
  // P_{vs}, Eq. (2)
  _saturation_pressure_of_water_vapor_over_ice[_qp] = (1.0/convert_meters) * _property_uo.saturationPressureOfWaterVaporOverIce(_temperature[_qp]);
}
